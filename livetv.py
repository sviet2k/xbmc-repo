...
def showMovies3(): #affiche les videos disponible du live
    ...
    if (aResult[0] == True):
        ...
        urls = []
        for aEntry in aResult[1]:
            urls.append('http:' + aEntry)
        
        for aEntry in multiDecode(urls):
            sUrl4 =  aEntry
            oHoster = cHosterGui().checkHoster(sUrl4)
            if (oHoster != False):
                oHoster.setDisplayName(sMovieTitle2) #nom affiche
                oHoster.setFileName(sMovieTitle2) #idem
                cHosterGui().showHoster(oGui, oHoster, sUrl4, '')
        """
        for aEntry in multiDecode(urls):
            sTitle = sMovieTitle2
            sUrl4 =  aEntry
...
def showHosters(): #affiche les videos disponible du live
    ...
    if (oHoster != False):
        oHoster.setDisplayName(sMovieTitle2) #nom affiche
        oHoster.setFileName(sMovieTitle2) #idem
        cHosterGui().showHoster(oGui, oHoster, sUrl4, sThumb)

    oGui.setEndOfDirectory()

def multiDecode(urls):
    ...
    with concurrent.futures.ThreadPoolExecutor ( max_workers = 5 ) as e:
        for url in urls:
            f.append(e.submit(_decode, url, UA))
...
def _decode(sUrl4, UA):
    ...
    try:
        ...
        if (aResult[0]):
            url = aResult[1][0]
            return _decode_host(url, UA, sUrl4, sHtmlContent)
    except: pass
    
    return sHosterUrl
    
def _decode_host(url, UA, sUrl4, sHtmlContent):
